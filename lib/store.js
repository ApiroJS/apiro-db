...
async function versionReview() {
  const res = await fetch('https://packages.krispowers.dev/packages/apiro-db.json')
  const data = await res.json()
  const ver = '1.2.0'
    
  // Check version
  if (data.version.recommended && data.version.latest !== ver) {
    console.log(`\n\n\x1b[33mYou're currently using & outdated version of Apiro-DB. Please review the following:\x1b[0m\nInstalled Version: \x1b[31m${ver}\n\x1b[0mLatest Version: \x1b[32m${data.version.latest}\x1b[0m\n\n`)
  }
}
